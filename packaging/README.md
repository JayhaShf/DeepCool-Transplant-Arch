...
`bash packaging/build-release.sh 0.1.1` 会先清空 `dist/`，然后生成：

| 文件 | 内容 |
|---|---|
| `deepcool-linux-port-0.1.1.tar.zst` | Electron 23、已打补丁应用、daemon、运行脚本和 `/opt` 安装器 |
| `deepcool-linux-port-0.1.1-source.tar.zst` | 仓库源码快照和 `BUILDING.md`；不含官方安装包或 npm 依赖 |
| `deepcool-linux-port-0.1.1-sbom.cdx.json` | CycloneDX 1.5 清单，覆盖 Electron 与应用树中可识别的 Node.js 包 |
...
```bash
bash packaging/build-release.sh 0.1.1
cd dist
...
```bash
tar --zstd -xf deepcool-linux-port-0.1.1.tar.zst
sudo bash deepcool-linux-port-0.1.1/packaging/install-tarball.sh \
  "$PWD/deepcool-linux-port-0.1.1"
deepcool-linux-port
```

安装器把应用放在 `/opt/deepcool-linux-port`，创建 `/usr/bin` 符号链接，安装桌面
入口，并调用 daemon 安装器。归档也可直接运行
`deepcool-linux-port-0.1.1/bin/deepcool-linux-port`，但这不会自动安装 daemon 或系统依赖。
...
```bash
sudo bash deepcool-linux-port-0.1.1/packaging/install-tarball.sh \
  "$PWD/deepcool-linux-port-0.1.1" --autostart

sudo bash deepcool-linux-port-0.1.1/packaging/install-tarball.sh \
  "$PWD/deepcool-linux-port-0.1.1" --autostart --user jay
```
...
- `dist/deepcool-linux-port-0.1.1-source.tar.zst`
- `dist/PKGBUILD`
